...
class Scheduler:
    ...
    async def _reminder_loop(self):
        ...
        while True:
            try:
                now = datetime.now(tz=LONDON_TZ)
                # Schedule first reminder at 21:15 London time, then every 15 minutes
                first_run = datetime.combine(now.date(), time(21, 15, tzinfo=LONDON_TZ))
                if now < first_run:
                ...
                if channel:
                    ...
                    embed = discord.Embed(
                        title=embed_title,
                        description="Remember to vote in the event poll!",
                        color=0xFFA500
                    )
