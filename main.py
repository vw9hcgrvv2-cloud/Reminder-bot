...
class Scheduler:
    ...
    async def _reminder_loop(self):
        ...
        while True:
            try:
                now = datetime.now(tz=LONDON_TZ)
                # Schedule first reminder at 21:30 London time, then every 15 minutes
                first_run = datetime.combine(now.date(), time(21, 30, tzinfo=LONDON_TZ))
                if now < first_run:
                ...
                if channel:
                    ...
                    # Keep the role ping in the message content
                    content = role_mention
                    embed = discord.Embed(
                        title=embed_title,
                        description="Remember to vote in the event poll! <#1524445184853803069>",
                        color=0xFFA500
                    )
