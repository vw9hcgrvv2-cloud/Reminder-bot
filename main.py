...
if not POLL_CHANNEL_ID:
    ...
    raise SystemExit(1)
POLL_CHANNEL_ID = _to_int(POLL_CHANNEL_ID, "POLL_CHANNEL_ID")

if not REMINDER_CHANNEL_ID:
    logger.error("REMINDER_CHANNEL_ID is not set.")
    raise SystemExit(1)
REMINDER_CHANNEL_ID = _to_int(REMINDER_CHANNEL_ID, "REMINDER_CHANNEL_ID")
...
# Persistent in-memory fallback (also persisted to file)
poll_state = load_state()

# Guard for starting background tasks only once
...
async def start_background_tasks_once():
    ...
    async with start_lock:
        ...
        # Use asyncio.create_task for compatibility with newer discord.py
...
async def daily_18_poll():
    ...
    while not bot.is_closed():
        ...
        view = PollView()
        try:
            # Delete previous poll message if possible
            ...
            if prev_id:
                try:
                    ...
                    logger.debug("Could not delete previous poll message (it may be too old or missing).")
