...
async def _safe_defer(interaction: discord.Interaction, ephemeral: bool = True):
    try:
        ...
        logger.debug(f"Defer skipped: {type(e).__name__}")
        ...
        try:
            ...
            if role_error_msg:
                await _safe_followup(interaction,
                    f"Vote recorded: {vote_type.replace('_', ' ').title()}\n⚠️ Role: {role_error_msg}")
            else:
                await _safe_followup(interaction,
                    f"Vote recorded: {vote_type.replace('_', ' ').title()}")

        except Exception as e:
            logger.error(f"Vote error | User {user_id}: {type(e).__name__}: {e}")
            await _safe_followup(interaction, "An error occurred recording your vote.")
...
class TomorrowPollView(View):
    ...
    async def handle_vote(self, interaction: discord.Interaction, vote_type: str):
        ...
        try:
            ...
            logger.error(f"Tomorrow vote error | User {user_id}: {type(e).__name__}: {e}")
            await _safe_followup(interaction, "An error occurred recording your vote.")
        ...
        try:
            ...
            if ping_role and ping_role in interaction.user.roles:
                ...
                if user_id not in state.state["opted_out_users"]:
                    ...
                    state.save()
                await _safe_followup(interaction, "✅ Event pings disabled. You will be re-enrolled when you vote in the next poll.")
            else:
                await _safe_followup(interaction, "You don't have the Event Ping! role.")
        except Exception as e:
            logger.error(f"Disable ping error | User {user_id}: {type(e).__name__}: {e}")
            await _safe_followup(interaction, "An error occurred.")
...
class TournamentBot(discord.Client):
    ...
    async def daily_1400_reminder(self):
        """14:00 reminder — sends to REMINDER_CHANNEL_ID with Event Ping! role mention"""
        try:
            channel = await self.get_reminder_channel()
            if not channel:
