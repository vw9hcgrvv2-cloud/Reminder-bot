...
class TournamentBot(discord.Client):
    def __init__(self) -> None:
        # ✅ FIX: Pass intents to parent class
    ...
    async def _send_reminder_message(self, rem_key: str) -> bool:
        async with self.reminder_send_lock:
            ...
            try:
                await ch.send(content, allowed_mentions=ALLOWED_MENTIONS_ROLES)
        ...
        async with self.task_locks["tomorrow_poll"]:
            ...
            if not valid:
                ...
                try:
                    ...
                    await state.save()
                    logger.info("✅ Created tomorrow's poll")
                except Exception as e:
...
if __name__ == "__main__":
    ...
    if not TOKEN:
        ...
        exit(1)
    # ✅ FIX: No intents argument here — handled inside class
    bot = TournamentBot()
    bot.run(TOKEN)
