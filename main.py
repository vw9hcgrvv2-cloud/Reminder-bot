...
class PollView(View):
    ...
    async def handle_vote(self, interaction: discord.Interaction, vote_type: str):
        ...
        try:
            ...
            state.save()
            await interaction.followup.send(f"Vote recorded: {vote_type.replace('_', ' ').title()}", ephemeral=True, allowed_mentions=ALLOWED_MENTIONS_NONE)
        except Exception as e:
            logger.error(f"Vote handling error: {e}")
            await interaction.followup.send("An error occurred while recording your vote.", ephemeral=True, allowed_mentions=ALLOWED_MENTIONS_NONE)
        ...
        try:
            ...
            if ping_role and ping_role in interaction.user.roles:
                try:
                    ...
                    await interaction.followup.send("Error removing role.", ephemeral=True, allowed_mentions=ALLOWED_MENTIONS_NONE)
            else:
            ...
            await interaction.followup.send("An error occurred.", ephemeral=True, allowed_mentions=ALLOWED_MENTIONS_NONE)
...
class TournamentBot(discord.Client):
    ...
    async def find_existing_tomorrow_poll(self, channel):
        """Search last 50 messages for a valid tomorrow poll embed"""
        tomorrow = state.get_tomorrow_key()
    ...
    async def create_startup_poll(self):
        logger.info("Checking startup poll…")
        try:
            tomorrow = state.get_tomorrow_key()
            ...
            logger.info("Creating new tomorrow poll")
