...
class AttendanceView(discord.ui.View):
    ...
    async def attend_yes(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(EPHEMERAL_ATTENDING, ephemeral=True)
    ...
    async def attend_maybe(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(EPHEMERAL_MAYBE, ephemeral=True)
    ...
    async def attend_no(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(EPHEMERAL_NOT_COMING, ephemeral=True)

# Helper to format simple attendance embed (used by poll posting)
async def _format_attendance_embed(mapping, guilds):
    ...
    embed.add_field(name="Responses", value="Click a button to respond.")
    return embed
...
class Scheduler:
    ...
    async def _reminder_loop(self):
        ...
        while True:
            try:
                now = datetime.now(tz=LONDON_TZ)
                next_run_min = (now.minute // 15) * 15
                next_run = now.replace(minute=next_run_min, second=0, microsecond=0)
                ...
                if channel:
                    embed = discord.Embed(
                        ...
                        description="Don't forget today's event! Tap the button to disable reminders if you wish.",
                        color=0xFFA500
                    )
