...
class Scheduler:
    ...
    async def _reminder_loop(self):
        ...
        while True:
            try:
                now = datetime.now(tz=LONDON_TZ)
                # Determine next target time: first at 19:00 LT, then every 15 minutes thereafter
                today_19 = datetime.combine(now.date(), time(19, 0, tzinfo=LONDON_TZ))
                if now < today_19:
                    delta = (today_19 - now).total_seconds()
                else:
                    # after 19:00, schedule on the next 15-minute boundary
