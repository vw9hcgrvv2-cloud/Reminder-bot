...
class Scheduler:
    ...
    async def _reminder_loop(self):
        ...
        while True:
            try:
                now = datetime.now(tz=LONDON_TZ)
                # Schedule first reminder at 19:30 London time, then every 15 minutes
                first_run = datetime.combine(now.date(), time(19, 30, tzinfo=LONDON_TZ))
                if now < first_run:
                ...
                if channel:
                    ...
                    role_mention = f"<@&{role.id}>" if role is not None else ""
                    if role_mention:
