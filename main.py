...
import asyncio
import logging
import aiofiles  # Async file I/O for non-blocking saves
...
async def _safe_defer(interaction: discord.Interaction, ephemeral: bool = True) -> bool:
    """Return True if deferred successfully, False otherwise."""
    try:
...
class StateManager:
    ...
    async def save(self):
        ...
        try:
            ...
            os.replace(temp_file, STATE_FILE)  # Atomic rename — fast & safe
        except Exception as e:
