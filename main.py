...
def _validate_state_dict(state_dict: Dict[str, Any]) -> None:
    ...
    list_fields = ["allowed_to_disable", "opted_out_users", "tomorrow_allowed_to_disable"]
...
def _build_poll_embed(title: str, voted_users: Dict[str, str]) -> discord.Embed:
    attending = []
    maybe = []
    not_attending = []
    for user_id, vote in voted_users.items():
        mention = f"<@{user_id}>"
        if vote == "attending":
            attending.append(mention)
        elif vote == "maybe":
            maybe.append(mention)
        elif vote == "not_attending":
            not_attending.append(mention)

    embed = discord.Embed(title=title, color=discord.Color.blue())
    if attending:
        embed.add_field(name="✅ Attending", value=", ".join(attending), inline=False)
    if maybe:
        embed.add_field(name="❓ Maybe", value=", ".join(maybe), inline=False)
    if not_attending:
        embed.add_field(name="❌ Not Attending", value=", ".join(not_attending), inline=False)
    total = len(voted_users)
    embed.set_footer(text=f"Total votes: {total} — Vote ✅ or ❓ to disable pings")
    return embed

async def _update_poll_display(bot: discord.Client, msg_id: int, title: str, voted_users: Dict[str, str], view: View):
    try:
        ch = bot.get_channel(EVENT_CHANNEL_ID) or await bot.fetch_channel(EVENT_CHANNEL_ID)
        if not ch:
            return
        msg = await ch.fetch_message(msg_id)
        new_embed = _build_poll_embed(title, voted_users)
        await msg.edit(embed=new_embed, view=view)
    except Exception as e:
        logger.debug(f"Could not update poll display: {e}")
...
class DisablePingView(View):
    ...
    async def disable_pings(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        try:
            ...
            # ✅ ALLOW IMMEDIATE ACCESS: voted attending/maybe on ANY active poll = allowed
            voted_today = user_id in state.state.get("voted_users", {})
            voted_tomorrow = user_id in state.state.get("tomorrow_voted_users", {})
            vote_today = state.state["voted_users"].get(user_id) if voted_today else None
            vote_tomorrow = state.state["tomorrow_voted_users"].get(user_id) if voted_tomorrow else None

            # Only ✅ Attending or ❓ Maybe grants permission
            has_permission = (vote_today in ("attending", "maybe")) or (vote_tomorrow in ("attending", "maybe"))

            if not has_permission:
                if deferred:
                    await interaction.followup.send("❌ Vote ✅ Attending or ❓ Maybe first to disable pings.", ephemeral=True)
                return
            ...
            if ping_role not in interaction.user.roles:
                if deferred:
                    await interaction.followup.send("You don't have the role to remove.", ephemeral=True)
                return
            ...
            if deferred:
                await interaction.followup.send("✅ Event Pings disabled! You will no longer be pinged for events.", ephemeral=True)
        except Exception as e:
    ...
    async def _handle_vote(self, interaction: discord.Interaction, vote_type: str, prefix: str) -> None:
        ...
        try:
            ...
            if prefix:
                ...
                poll_msg_id = state.state.get("tomorrow_poll_msg_id")
                voted_key = "tomorrow_voted_users"
                poll_title = TOMORROW_POLL_TITLE
            else:
                target_date = state.get_today_key()
                session_date = state.state.get("poll_session_date")
                poll_msg_id = state.state.get("current_poll_msg_id")
                voted_key = "voted_users"
            ...
            if interaction.message and poll_msg_id != interaction.message.id:
                logger.info("⏭️ Vote rejected: message ID mismatch")
                if deferred:
                    await interaction.followup.send("Wrong poll.", ephemeral=True)
                return

            voted_users = state.state[voted_key]

            # ✅ Voting ✅ or ❓ grants permission to disable pings IMMEDIATELY
            ...
            # Update poll display with voter list
            if self.bot_ref and poll_msg_id:
                asyncio.create_task(_update_poll_display(self.bot_ref, poll_msg_id, poll_title, voted_users, self))

            if deferred:
                if vote_type in ("attending", "maybe"):
                    await interaction.followup.send(f"✅ Vote recorded: **{vote_type.replace('_',' ').title()}**\n\nYou can now use the **Disable Event Pings** button below!", ephemeral=True)
                else:
                    await interaction.followup.send(f"✅ Vote recorded: **{vote_type.replace('_',' ').title()}**\n\nNote: Vote ✅ Attending or ❓ Maybe to unlock Disable Event Pings.", ephemeral=True)

            # Auto-grant/remove Event Attendee role
            attendee_role = discord.utils.get(guild.roles, name=ATTENDEE_ROLE_NAME)
...
class PollView(BasePollView):
    def __init__(self, bot_ref=None):
        super().__init__(bot_ref=bot_ref)
        # ✅ Add Disable Ping button to today's poll
        self.add_item(discord.ui.Button(label="Disable Event Pings", style=discord.ButtonStyle.gray, custom_id="disable_pings"))
...
class TomorrowPollView(BasePollView):
    def __init__(self, bot_ref=None):
        super().__init__(bot_ref=bot_ref)
        # ✅ Add Disable Ping button to tomorrow's poll
        self.add_item(discord.ui.Button(label="Disable Event Pings", style=discord.ButtonStyle.gray, custom_id="disable_pings"))
...
class TournamentBot(discord.Client):
    ...
    async def setup_hook(self) -> None:
        self.add_view(DisablePingView())
        self.add_view(PollView(bot_ref=self))
        self.add_view(TomorrowPollView(bot_ref=self))
    ...
    async def _send_reminder_message(self, rem_key: str) -> bool:
        async with self.reminder_send_lock:
            ...
            if not ch:
                ...
                await state.save()
                logger.warning(f"⏭️ {rem_key} reminder deferred: channel unavailable")
                return False
            ping_role = discord.utils.get(ch.guild.roles, name=PING_ROLE_NAME)
            if not ping_role:
                state.state["pending_reminders"][rem_key]["failures"] = failures + 1
                await state.save()
                logger.warning(f"⏭️ {rem_key} reminder deferred: role not found")
                return False
            content = f"{ping_role.mention} Don't forget to vote in the attendance poll! Head over to <#{EVENT_CHANNEL_ID}> now."
            try:
                # ✅ Attach Disable Ping button to EVERY 15-minute reminder message
                reminder_view = DisablePingView()
                await ch.send(content, allowed_mentions=ALLOWED_MENTIONS_ROLES, view=reminder_view)
                state.state["sent_reminders"][rem_key] = True
                state.state["pending_reminders"].pop(rem_key, None)
                self._seen_boundaries.add(rem_key)
                await state.save()
                logger.info(f"✅ Sent {rem_key} reminder with Disable Ping button")
                return True
            except Exception as e:
                state.state["pending_reminders"][rem_key]["failures"] = failures + 1
    ...
    async def _1800_create_poll(self) -> None:
        async with self.task_locks["1800"]:
            ...
            embed = _build_poll_embed(TODAY_POLL_TITLE, {})
            try:
                # ✅ PollView already includes Disable Ping button
                msg = await ch.send(embed=embed, view=PollView(bot_ref=self))
                state.state["current_poll_msg_id"] = msg.id
                await state.save()
                logger.info("✅ Created today's poll with Disable Ping button")
            except Exception as e:
        ...
        async with self.task_locks["tomorrow_poll"]:
            ...
            if stored_id and stored_target == tomorrow:
                try:
                    ...
                    valid = True
                    logger.info("✅ Found existing tomorrow poll")
                except NotFound:
            ...
            if not valid:
                embed = _build_poll_embed(TOMORROW_POLL_TITLE, {})
                try:
                    # ✅ TomorrowPollView already includes Disable Ping button
                    msg = await ch.send(embed=embed, view=TomorrowPollView(bot_ref=self))
                    ...
                    await state.save()
                    logger.info("✅ Created tomorrow's poll with Disable Ping button")
                except Exception as e:
    ...
    async def _scheduler_loop(self) -> None:
        ...
        while not self.is_closed():
            try:
                ...
                if now.hour == 14 and 5 <= now.minute <= 10 and state.state.get("last_1400_run_date") != today:
                    state.state["last_1400_run_date"] = today
                    await self._1400_announcement()
                    await state.save()

                if now.hour == 15 and 5 <= now.minute <= 10 and state.state.get("last_1500_reset_date") != today:
                    state.state["last_1500_reset_date"] = today
                    await self._1500_daily_reset()
                    await state.save()

                if now.hour == 18 and 5 <= now.minute <= 10 and state.state.get("last_1800_poll_date") != today:
