...
# ✅ THIS VIEW IS USED ONLY ON REMINDERS — LEFT COMPLETELY UNTOUCHED
class DisablePingView(View):
    ...
    async def disable_pings(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        # ✅ FIXED: Vote in TODAY'S poll = INSTANT permission to disable pings
        ...
        has_permission = vote_today in ("attending", "maybe")

        if not has_permission:
            try:
                await interaction.followup.send("❌ Vote ✅ Attending or ❓ Maybe in today's poll first to disable pings.", ephemeral=True)
            except Exception:
                pass
            return
...
class BasePollView(View):
    ...
    async def _handle_vote(self, interaction: discord.Interaction, vote_type: str, prefix: str) -> None:
        ...
        if vote_type in ("attending", "maybe"):
            msg = f"✅ Vote recorded: **{vote_type.replace('_',' ').title()}**"
        else:
            msg = f"✅ Vote recorded: **{vote_type.replace('_',' ').title()}**"
        try:
...
class PollView(BasePollView):
    def __init__(self, bot_ref=None):
        ...
        # ❌ Disable Ping Button REMOVED from today's poll
...
class TomorrowPollView(BasePollView):
    def __init__(self, bot_ref=None):
        super().__init__(bot_ref=bot_ref)
        # ❌ Disable Ping Button REMOVED from tomorrow's poll
...
class TournamentBot(discord.Client):
    ...
    async def setup_hook(self) -> None:
        self.add_view(DisablePingView())  # ✅ Still registered for reminders ONLY
    ...
    async def _send_reminder_message(self, rem_key: str) -> bool:
        async with self.reminder_send_lock:
            ...
            try:
                # ✅ DisablePingView REMAINS on reminders — completely unchanged
    ...
    async def _first_reminder_worker(self) -> None:
        ...
        try:
            now = datetime.now(LONDON_TZ)
            # ✅ Changed from every 15 min → EVERY 60 MINUTES (on the hour)
            minutes_next = 60 - now.minute if now.minute != 0 else 0
            next_hour = now.hour + (1 if minutes_next else 0)
            next_boundary = now.replace(hour=next_hour % 24, minute=0, second=0, microsecond=0)
    ...
    async def _1800_create_poll(self) -> None:
        async with self.task_locks["1800"]:
            ...
            try:
                # ✅ Today poll — NO Disable Ping button (only vote buttons)
                msg = await ch.send(
                    "@everyone New attendance poll is now open! 📊",
                    embed=embed,
                    view=PollView(bot_ref=self),
                    allowed_mentions=ALLOWED_MENTIONS_EVERYONE
                )
                state.state["current_poll_msg_id"] = msg.id
                await state.save()
                logger.info("✅ Created today's poll (no disable button on poll)")
            except Exception as e:
    ...
    async def _startup_poll(self) -> None:
        async with self.task_locks["tomorrow_poll"]:
            ...
            if not valid:
                ...
                try:
                    # ✅ Tomorrow poll — NO Disable Ping button (only vote buttons)
                    msg = await ch.send(
                        "@everyone Attendance poll for tomorrow is now open! 📊",
                        embed=embed,
                        view=TomorrowPollView(bot_ref=self),
                        allowed_mentions=ALLOWED_MENTIONS_EVERYONE
                    )
                    ...
                    await state.save()
                    logger.info("✅ Created tomorrow's poll (no disable button on poll)")
                except Exception as e:
    ...
    async def _scheduler_loop(self) -> None:
        ...
        while not self.is_closed():
            try:
                ...
                # ✅ CHANGED: Reminders every 60 minutes (ON THE HOUR) instead of every 15 min
                if now.minute == 0 and 5 <= now.second <= 35:
