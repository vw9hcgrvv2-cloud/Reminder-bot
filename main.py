...
class Scheduler:
    def __init__(self, bot: commands.Bot):
        ...
        # Removed test loop task attributes
        # Removed startup flags/locks
    ...
    async def _reminder_loop(self):
        ...
        while True:
            try:
                now = datetime.now(tz=LONDON_TZ)
                # Schedule first reminder at 21:00 London time, then every 15 minutes
                first_run = datetime.combine(now.date(), time(21, 0, tzinfo=LONDON_TZ))
                if now < first_run:
                    delta = (first_run - now).total_seconds()
                else:
                    # compute next 15-minute boundary after now
                ...
                if channel:
                    ...
                    # Do not mention everyone or Event Ping!; only tag the event channel in description
