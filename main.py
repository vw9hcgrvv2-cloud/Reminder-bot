...
# ─── CRITICAL: DEFER BUTTONS PROPERLY ───
async def _defer_button(interaction: discord.Interaction):
    """✅ Use defer_update for buttons — FAST, correct method."""
    try:
        if not interaction.response.is_done():
            await interaction.response.defer_update()  # ⚠️ THIS IS THE FIX — NOT defer(ephemeral=True)
        return True
...
# ─── SAFE FOLLOWUP RESPONSE ───
async def _followup(interaction: discord.Interaction, content: str):
    """Send ephemeral confirmation AFTER defer_update."""
    try:
        await interaction.followup.send(content, ephemeral=True)
    except discord.NotFound:
        logger.warning("Interaction token expired — cannot respond")
    except Exception as e:
...
async def _manage_role_safe(member, role, add: bool, timeout_sec: float = 2.5):
    ...
    if role.position >= bot_member.top_role.position:
        return False, "Role hierarchy issue"
    try:
        if add:
...
class PollView(View):
    ...
    async def attending(self, interaction: discord.Interaction, button: discord.ui.Button):
        await _defer_button(interaction)  # ✅ INSTANT DEFER — FIRST LINE
    ...
    async def maybe(self, interaction: discord.Interaction, button: discord.ui.Button):
        await _defer_button(interaction)
    ...
    async def handle_vote(self, interaction: discord.Interaction, vote_type: str):
        ...
        if not guild or state.state.get("poll_session_date") != today:
            await _followup(interaction, "This poll is no longer active.")
            return
        ...
        await state.save()
        logger.info(f"Vote saved: {user_id} → {vote_type}")

        # ✅ RESPOND TO USER — FAST
        await _followup(interaction, f"Vote recorded: {vote_type.replace('_', ' ').title()}")

        # ✅ ROLE UPDATES HAPPEN AFTER RESPONSE — NO TIMEOUT RISK
        if attendee_role:
            try:
                if vote_type in ("attending", "maybe"):
                ...
                logger.debug(f"Role update bg: {e}")
...
class TomorrowPollView(View):
    ...
    async def handle_vote(self, interaction: discord.Interaction, vote_type: str):
        ...
        if not guild or state.state.get("tomorrow_poll_created_date") != tomorrow:
            await _followup(interaction, "This poll has expired.")
            return
...
class DisablePingView(View):
    ...
    async def handle_disable(self, interaction: discord.Interaction):
        ...
        if not guild:
            await _followup(interaction, "Guild not found.")
            return
        ...
        if not voted_today and not voted_tomorrow:
            await _followup(interaction, "You must vote in a poll before disabling pings!")
            return

        ping_role = discord.utils.get(guild.roles, name=PING_ROLE_NAME)
        if not (ping_role and ping_role in interaction.user.roles):
            await _followup(interaction, "You don't have the Event Ping! role.")
            return

        success, err_msg = await _manage_role_safe(interaction.user, ping_role, add=False)
        if not success:
            await _followup(interaction, f"Could not remove role: {err_msg}")
            return
        ...
        await _followup(interaction, "✅ Event pings disabled. You will be re-enrolled when you vote in the next poll.")
...
class TournamentBot(discord.Client):
    ...
    async def create_startup_poll(self):
        ...
        try:
            ...
            logger.error(f"Startup poll failed: {e}")
    ...
    async def assign_ping_role_to_eligible(self, guild):
        ...
        if not ping_role:
            return
        bot_member = guild.me
        if not bot_member.guild_permissions.manage_roles or ping_role.position >= bot_member.top_role.position:
            return
        opted_out = set(state.state.get("opted_out_users", []))
        for member in guild.members:
            if member.bot or str(member.id) in opted_out or ping_role in member.roles:
                continue
            await _manage_role_safe(member, ping_role, add=True)
