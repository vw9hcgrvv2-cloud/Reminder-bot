...
class DisablePingView(View):
    ...
    async def disable_pings(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        # ✅ INSTANT DEFER — FIXES "TOOK TOO LONG TO RESPOND"
        try:
...
class BasePollView(View):
    ...
    async def _handle_vote(self, interaction: discord.Interaction, vote_type: str, prefix: str) -> None:
        # ✅ INSTANT DEFER — prevents timeout
        try:
            if not interaction.response.is_done():
                await interaction.response.defer(ephemeral=True, thinking=False)
        except Exception:
        ...
        if vote_type in ("attending", "maybe"):
            msg = f"✅ Vote recorded: **{vote_type.replace('_',' ').title()}**\n\nYou can now use the **Disable Event Pings** button below!"
        else:
            msg = f"✅ Vote recorded: **{vote_type.replace('_',' ').title()}**\n\nNote: Vote ✅ Attending or ❓ Maybe to unlock Disable Event Pings."
        try:
...
class TournamentBot(discord.Client):
    ...
    async def _1800_create_poll(self) -> None:
        async with self.task_locks["1800"]:
            ...
            try:
                # ✅ @everyone EVERY TIME poll is posted
                msg = await ch.send(
                ...
                await state.save()
                logger.info("✅ Created today's poll with @everyone")
            except Exception as e:
    ...
    async def _startup_poll(self) -> None:
        async with self.task_locks["tomorrow_poll"]:
            ...
            if not valid:
                ...
                try:
                    # ✅ @everyone EVERY TIME tomorrow poll is posted
                    msg = await ch.send(
                    ...
                    await state.save()
                    logger.info("✅ Created tomorrow's poll with @everyone")
                except Exception as e:
